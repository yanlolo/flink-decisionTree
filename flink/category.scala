...
object WordCount {
  ...
  def main(args: Array[String]) {
    ...
    val re = parCat(labledSample)
    re.map { s => (s.position, s.label, s.feature.toList) }.writeAsText("/home/hadoop/Desktop/test/result")
  ...
  case class GainStr(position: String, featureIndex: Int, featureValue: String, gain: Int)
  ...
  def parCat(labledSample: DataSet[LabeledVectorStr]): DataSet[LabeledVectorStr] = {
    ...
    //gain
    val gainPre1 = histoSample.groupBy("position", "label", "featureIndex").reduce {
      (s1, s2) =>
        new HistogramStr(s1.position, s1.label, s1.featureIndex, " ", s1.frequency + s2.frequency)
    }

    val gainPre2 = gainPre1.groupBy("position", "featureIndex").reduce {
      (s1, s2) =>
        new HistogramStr(s1.position, 0, s1.featureIndex, " ", s1.frequency + s2.frequency)
    }

    val entropy = gainPre1.join(gainPre2).where("position", "featureIndex").equalTo("position", "featureIndex") {
      (gainPre1, gainPre2, out: Collector[HistogramStr]) =>
        out.collect(new HistogramStr(gainPre1.position, gainPre1.label, gainPre1.featureIndex, " ", gainPre1.frequency / gainPre2.frequency))
    }.map {
      s =>
        {
    ...
    val gain = entropy.groupBy("position", "featureIndex").reduce {
      (s1, s2) =>
        new HistogramStr(s1.position, 0, s1.featureIndex, " ", s1.frequency + s2.frequency)
    }
    ...
    // gain right
    val gainRightPre1 = gainPre1.join(gainLeftPre1).where("position", "label", "featureIndex").equalTo("position", "label", "featureIndex") {
      (g1, g2, out: Collector[HistogramStr]) =>
        out.collect(new HistogramStr(g1.position, g1.label, g1.featureIndex, g2.featureValue, g1.frequency - g2.frequency))
    }
    ...
    //delta
    val deltaPre = histoSample.groupBy("position", "featureIndex").reduce {
      (s1, s2) => new HistogramStr(s1.position, 0, s1.featureIndex, "", s1.frequency + s2.frequency)
    }

    val delta = histoSample.join(deltaPre).where("position", "featureIndex").equalTo("position", "featureIndex") {
      (s1, s2, out: Collector[HistogramStr]) =>
        out.collect(new HistogramStr(s1.position, 0, s1.featureIndex, s1.featureValue, s1.frequency / s2.frequency))
    }

    val gainInfo = gain.join(gainLeft).where("position", "featureIndex").equalTo("position", "featureIndex")
      .map { s => (s._1.position, s._1.featureIndex, s._2.featureValue, s._1.frequency, s._2.frequency) }
      .join(gainRight).where("position", "featureIndex", "featureValue").equalTo("position", "featureIndex", "featureValue")
      .map { s => (s._2.position, s._2.featureIndex, s._2.featureValue, s._1._4, s._1._5, s._2.frequency) }
      .join(delta).where("position", "featureIndex", "featureValue").equalTo("position", "featureIndex", "featureValue")
    ...
    val splitedSample: DataSet[LabeledVectorStr] = labledSample.join(splitPlace).where("position").equalTo("position")
      .map { s =>
        if (s._1.feature(s._2._2) == s._2.gain)
