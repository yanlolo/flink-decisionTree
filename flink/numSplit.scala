...
object WordCount {
  ...
  def main(args: Array[String]) {
    ...
    var i = 1
    var result = partition(labledSample, i)
    var inputs: DataSet[LabeledVector] = result._1

    i = 2
    result = partition(inputs, i)
    var splitedSample: DataSet[LabeledVector] = result._1
  ...
  private val numBins = 1483 // B bins for Update procedure
  private val numSplit = 1483 //By default it should be same as numBins
  private val numLevel = 2 // how many levels of tree
   ...
   * partition one level
   */
  def partition(labledSample: DataSet[LabeledVector], i: Int): (DataSet[LabeledVector], DataSet[(String, Int, Double)]) = {
    ...
    updatedSample.map { s => (s.position, s.label, s.featureIndex, s.histo.toList) }.writeAsText("/home/hadoop/Desktop/test/updatedSample" + i)
    ...
    mergedSample.map { s => (s.position, s.featureIndex, s.histo.toList) }.writeAsText("/home/hadoop/Desktop/test/mergedSample" + i)
    ...
    uniform.map { s => (s.position, s.featureIndex, s.uniform.toList) } writeAsText ("/home/hadoop/Desktop/test/uniform" + i)
    ...
    sum.map { s => (s.position, s.label, s.featureIndex, s.sum.toList) }.writeAsText("/home/hadoop/Desktop/test/sum" + i)

    val gain: DataSet[Gain] = gainCal(labledSample, sum)
    gain.map { s => (s.position, s.featureIndex, s.gain.toList) } writeAsText ("/home/hadoop/Desktop/test/gain" + i)

    val splitPlace: DataSet[(String, Int, Double)] = findSplitPlace(gain, uniform)
    splitPlace.writeAsText("/home/hadoop/Desktop/test/splitPlace" + i)
