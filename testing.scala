...
object input {
   ...
   */
  def dataPro(data: ArrayBuffer[ArrayBuffer[Double]]): Array[Map[Double, Double]] = {
    // only allocated when data is not empty
    var labelNums = new collection.mutable.HashMap[Double, Int]
    var clsData = new Array[Map[Double, Double]](data(0).size - 2)

    for (dd <- data) {// loop for all data by line
      for (i <- 1 to dd.size - 1) { // loop for features by column
        clsData(i - 1) += (dd(0) -> dd(i))  //??? same key????
      }
    }
    clsData
  }
